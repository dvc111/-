...
本实现参考：
Song Wang et al. *Reasoning of Large Language Models over Knowledge Graphs with Super-Relations*. ICLR 2025.

对应关系：
- Super-Relations → `hyper_relation_scoring.py` 超关系语义评分；
- Forward / backward search 思想 → `macro_subgraph.py` 宏观子图裁剪；
- Retrieval efficiency optimization → `pipeline.py` 分层检索接口。

本版本针对软著一需求进行了工程化调整：
- 使用本地 KG 替代大型 Freebase/Wikidata 查询；
- 保留 ReKnoS 的超关系检索核心思想；
- 输出标准化 macro_subgraph，便于后续 GNN 模块接入。
