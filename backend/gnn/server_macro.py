...
def mock_llm(p):
    ...
    if "华法林和什么药有相互作用" in p: return "华法林"
    if "布洛芬会导致什么" in p: return "布洛芬"
    if "阿司匹林有什么副作用" in p: return "阿司匹林"
    if "阿司匹林属于什么类别的药" in p: return "阿司匹林"
    if p.count("阿司匹林和布洛芬同时服用会有什么风险") >= 2: return "阿司匹林\n布洛芬"
    if "对乙酰氨基酚" in p: return "对乙酰氨基酚"
    return ""
