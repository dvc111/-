"""GNN introspection for the micro demo (evidence subgraph, DDE features, message passing)."""
...
def run_micro_introspection(model, graph_data, triples, relation_id_map=None,
                            entity_labels=None, relation_labels=None,
                            topic_entity_ids=None, text_dim=None, dde_dim=None,
                            question_embedding=None, device=None):
    node_features = graph_data.node_features
    if text_dim is None or dde_dim is None:
        text_dim = int(node_features.size(-1)) - 10
        dde_dim = int(node_features.size(-1)) - text_dim
    ...
    x = node_features.to(target_device)
    ...
    for i, eid in enumerate(node_ids):
        feat = node_features[i].tolist()
        text_vec = feat[:text_dim]
        dde_vec = feat[text_dim:text_dim + dde_dim]
        nodes.append({
            "entity_id": eid,
            "label": node_labels[eid],
            "is_topic": eid in topic_set,
            "semantic_score": round(_cosine(qvec, text_vec), 4) if qvec else None,
            "neighbor_count": neighbor_count[eid],
            "edge_count": edge_count[eid],
            "text_dim": text_dim,
            "dde_dim": dde_dim,
            "dde_norm": round(math.sqrt(sum(v * v for v in dde_vec)), 6) if dde_vec else 0.0,
            "dde_mean": round(sum(dde_vec) / len(dde_vec), 6) if dde_vec else 0.0,
            "input_dim": text_dim + dde_dim,
            "importance": 0.0,
        })
    ...
    hidden_dim = getattr(model, "hidden_dim", int(x.size(-1)))
    return {
        "feature_spec": {
            "text_dim": text_dim,
            "dde_dim": dde_dim,
            ...
            "structure_source": "directional_distance_encoding",
        },
        "nodes": nodes,
        "edges": edges,
